...
def test_service_returns_the_index_passage_result(tmp_path: Path) -> None:
    from yt_insights.search.service import SearchService
    from yt_insights.search.sqlite_fts import SQLiteFtsIndex
    ...
    passage = Passage(
        passage_id=compute_passage_id(document_id, 0, 0.0, 4.0, "delegated passage"),
        document_id=document_id,
        ordinal=0,
        start_seconds=0.0,
        end_seconds=4.0,
        text="delegated passage",
        youtube_url=youtube_url(document.video_id, 0.0),
    )
    index = SQLiteFtsIndex(tmp_path / "search.sqlite3")
    index.rebuild(
    ...
    assert SearchService(index).get_passage(passage.passage_id) == (document, passage)
