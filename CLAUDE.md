...
| `output/speakers.md` | Speaker names extracted by regex from video titles (`by X`, `par X`, `with X`, `X @ Company`, etc.), grouped by channel. Regenerated by `scripts/build_speakers.py`, no LLM call. Reliable on conference/interview channels (devoxx, aidevcon, techready, devwithai, pragmaticengineer, martignole, tpc); near-empty on single-host channels whose titles don't name a guest (bloomberg, co-cto, alafrench, methode-aristote, optimisemonespace, ifttd), a title-format gap, not proof those channels lack guests |
| `output/llms.txt` | Hand-curated narrative synthesis (corpus descriptions, cross-corpus convergences). Review it after every channel acquisition; `run-channel.sh` fails finalization when the new slug is absent |
...
**Adding a channel**: use the `/yt-add-channel` skill, which drives `runbook/run-channel.sh` (a template, see `runbook/README.md` for the cookie/truncation/language-dedup pitfalls it encodes), then verifies `output/CATALOG.yaml`, `output/catalog.sqlite3`, `output/INDEX.md`, `output/llms.txt`, and `output/speakers.md`. `llms.txt` requires an editorial review for every new channel; the other four are rebuilt automatically. Never hand-roll the `yt-insights run` invocation for a full channel.

---

## Machine-Readable Documentation

Load `@llms.txt` for the tracked project snapshot: command map, modules,
storage schema, constraints, generated outputs, and open gaps. Architecture
rationale and the phased implementation plan are tracked under
`@docs/superpowers/`.
