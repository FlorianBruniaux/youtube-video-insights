...
**Adding a channel**: use the `/yt-add-channel` skill, which drives `runbook/run-channel.sh` (a template, see `runbook/README.md` for the cookie/truncation/language-dedup pitfalls it encodes), rebuilds the Markdown/YAML indexes, then refreshes the local SQLite catalog with `yt-insights catalog import-corpus output`. Never hand-roll the `yt-insights run` invocation for a full channel.
